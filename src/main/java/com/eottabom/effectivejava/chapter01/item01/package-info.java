 ...
 *
 * ---
 * 열거 타입은 인스턴드가 하나만 만들어짐을 보장한다.
 * 같은 객체가 자주 요청되는 상황이면 flyweight 패턴을 사용할 수 있다.
 * Java 8 부터는 인터페이스가 정적 메서드를 가질 수 없다는 제한이 풀려서, 인스턴스화 불가 동반 클래스를 둘 이유가 없다.
 * 서비스 제공자 프레임워크를 만드는 근간이 된다.
 * 서비스 제공자 인터페이스가 없으면, 각 구현체를 인스턴스로 만들 때 리플랙션을 사용해야한다.
 */
